...
class Popup extends Component {
	...
	start({ ...config }) {
		this.setState({
			...
			closeBtn: config.closeBtn !== undefined ? config.closeBtn : this.defaultCallback(),
	...
	handleImage(type) {
		switch (type) {
			...
			case 'Survey': return require('../../assets/survey.png')
	...
	render() {
		const { title, type, textBody, button, image, closebuttonText, buttonText, callback, closeBtn, background } = this.state
		let el = null;
		...
		if (this.state.closebutton) {
			cb = <TouchableOpacity style={styles.ButtonClose} onPress={closeBtn}>
				<Text style={styles.ButtonCloseText}>{closebuttonText}</Text>
			</TouchableOpacity>
		}
		else {
			cb = <Text></Text>
				...
				>
					<View style={styles.Header} />
					{
						this.state.icon ? (this.state.icon) :
							<Image
								source={image == null ? this.handleImage(type) : image}
								resizeMode="contain"
								style={styles.Image}
							/>
					}
